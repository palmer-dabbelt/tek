...
void processor_soffice_boot(void *context)
{
    p_name = talloc_strdup(context, "OODRAW");
}
             ...
             struct stack *s, struct makefile *m)
{
    ...
    p = talloc_get_type(p_uncast, struct processor_soffice);
    ...
    pdffile = talloc_strdup(c, outname);
    pdffile[strlen(pdffile) - 8] = '\0';
    strcat(pdffile, ".pdf");
    ...
    makefile_nam_cmd(m, "echo -e \"OODRAW\\t%s\"", infile);
    makefile_add_cmd(m, "mkdir -p \"%s\" >& /dev/null || true", cachedir);
    makefile_add_cmd(m, "lodraw --headless --convert-to pdf \"%s\" --outdir \"%s\" > /dev/null",
                     infile, cachedir);
    makefile_add_cmd(m, "mv \"%s\" \"%s\"",
                     pdffile, outname);
    makefile_end_cmds(m);
