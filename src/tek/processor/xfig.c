...
void processor_xfig_boot(void *context)
{
    p_name = talloc_strdup(context, "CONVERT");
}

struct processor *processor_xfig_search(void *context, const char *filename)
{
    struct processor_xfig *p;

    p = NULL;

    if (string_ends_with(filename, ".fig.pdf"))
        p = talloc(context, struct processor_xfig);
             ...
             struct stack *s, struct makefile *m)
{
    ...
    p = talloc_get_type(p_uncast, struct processor_xfig);
    ...
    makefile_nam_cmd(m, "echo -e \"XFIG\\t%s\"", infile);
    makefile_add_cmd(m, "mkdir -p \"%s\" >& /dev/null || true", cachedir);
    makefile_add_cmd(m, "fig2dev -L pdf -p 0 \"%s\" > \"%s\"", infile,
                     filename);
    makefile_end_cmds(m);
