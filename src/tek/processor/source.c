...
void processor_source_boot(void *context)
{
    p_name = talloc_strdup(context, "HILITE");
}

struct processor *processor_source_search(void *context, const char *filename)
{
    struct processor_source *p;

    p = NULL;

    if (string_ends_with(filename, ".c"))
        p = talloc(context, struct processor_source);
             ...
             struct stack *s, struct makefile *m)
{
    ...
    makefile_nam_cmd(m, "echo -e \"HILIGHT\\t%s\"", infile);
    makefile_add_cmd(m, "mkdir -p \"%s\" >& /dev/null || true", cachedir);
    makefile_add_cmd(m, "cp \"%s\" \"%s\" >& /dev/null || true", infile,
                     cachedir);
    makefile_add_cmd(m,
                     "source-highlight -i \"%s\" -o \"%s\".tmp -f latexcolor",
                     infile, filename);
    makefile_add_cmd(m, "echo \"\\texttt{\" > \"%s\"", filename);
    makefile_add_cmd(m, "cat \"%s\".tmp >> \"%s\"", filename, filename);
    makefile_add_cmd(m, "echo \"}\" >> \"%s\"", filename);
    makefile_add_cmd(m, "rm \"%s\".tmp", filename);
    makefile_end_cmds(m);
